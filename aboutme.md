---
layout: page
title: About me
subtitle: Economics, statistics, and data science
---

I am **Haoxuan (Mark) Sun**, a senior at [Wake Forest University](https://www.wfu.edu/) in Winston-Salem, North Carolina. I am completing a **B.S. in Economics** with minors in **Statistics** and **Mathematics** (May 2026). From **August 2026** I will begin an **M.Sc. in Statistics** at the [National University of Singapore](https://www.nus.edu.sg/). I am interested in roles and projects at the intersection of **economics, econometrics, and data science**.

### Education

- **National University of Singapore** — M.Sc. in Statistics (beginning Aug 2026), Singapore.
- **Wake Forest University** — B.S. Economics; minors in Statistics and Mathematics (Aug 2023 – May 2026). Cumulative GPA **3.83/4.00**; **Dean’s List** each semester; member of **Omicron Delta Epsilon** (economics honor society). Major coursework includes econometrics, machine learning, computer programming, optimization theories, and microeconomics.

### Experience

- **Research assistant** — Wake Forest Department of Economics, with Professor Koleman Strumpf (Sept 2025 – May 2026), Winston-Salem, NC. Supported econometric research by building automated **Python** pipelines to process and filter multi-terabyte TV advertising datasets; managed data extraction and consolidation in **Jupyter/Colab** for analysis of advertising markets.
- **Project intern, Global Production Planning** — Dell Technologies (June – Aug 2025), Xiamen, China. Simulated end-to-end production lead times (order-to-delivery) using an **XGBoost** machine learning model to forecast customer order timelines; improved simulation accuracy from **109%** (manual calculation from distribution) to **104%**. Improved forecasts by classifying lead-time factors (supplier-driven vs. customer-driven), running separate simulations, and grouping products by **CFS** levels.
- **Field sales and operations support intern** — DB Schenker Greater China (July – Aug 2024), Shanghai. Assisted the field sales team with quotes, proposals, and presentations tailored to customer requirements; maintained and updated customer databases for interactions, contracts, and account details.

### Data science project

- **NBA player salary prediction** (statistics learning group project; Feb 2026 – present) — Built a predictive model in **R** for 2022–2023 NBA player salaries from on-court performance metrics, with final prediction error of roughly **12%**. Used exploratory analysis, log transformations for variance stability, and **k-fold cross-validation** with **KNN**; extending the pipeline with penalized regression.

### Skills, languages, and interests

- **Skills:** R, Python, Java, SQL, data visualization, machine learning, Microsoft Office.
- **Languages:** Mandarin — native; English — native; German — basic.
- **Interests:** Soccer, Brazilian jiu-jitsu, Formula 1, basketball.

### Contact

Reach me through the links in the footer (email, LinkedIn, GitHub, U.S. phone). You can also use **(+86) 138-1744-1278** or **(+1) 704-502-9225**. For **Wake Forest Economics**, I am affiliated with the [Department of Economics](https://economics.wfu.edu/).

### This site

Built with [Beautiful Jekyll](https://beautifuljekyll.com/) on [GitHub Pages](https://pages.github.com/). Live site: [marksun04hx.github.io](https://marksun04hx.github.io). Source: [MarkSun04HX.github.io](https://github.com/MarkSun04HX/MarkSun04HX.github.io).
