...
**Shrinkage:** using \(\eta < 1\) (e.g. \(0.01\)–\(0.3\)) **slows** learning so each tree does a small correction; often you use **more trees** and get better generalization—similar in spirit to small steps in gradient descent.
...
$$
g_i = \left.\frac{\partial \, \ell(y_i, \hat{y})}{\partial \hat{y}}\right|_{\hat{y}^{(t-1)}_i}, \qquad
...
| **Gradient boosting** | Each new model fits **negative gradient** (pseudo-residuals) of the loss w.r.t. current predictions; general loss, not just squared error. |
| **XGBoost** | Gradient boosting + **regularized** tree objective (\(\gamma\), \(\lambda\)), **second-order** approximation (\(g_i, h_i\)), and **fast approximate** split search at scale. |
