...
**Repo (R):** Entry point **`Rscript scripts/setup.R`** — e.g. **`process-meps`** (optional **`--download`** for Stata PUF zips), **`pool`**, **`expand-exclusions`**, **`linear-baselines`**, **`install`** — producing yearly and **pooled** Parquets under **`data/processed/`**, manifests, and expanded exclusion lists (**`R/meps_competition_exclusions.R`**). **`scripts/tuning/build_selection_data.R`** builds a **train / test holdout** (defaults **10k / 2k** rows), **median-imputes** using **train** statistics only, fits **PCA with `irlba` on the training matrix**, projects test rows with **`predict`**, and writes **`selection_train.parquet`**, **`selection_test.parquet`**, and **`selection_data.parquet`** (training-only copy for older scripts). **`scripts/tuning/`** also holds **lasso / elastic net / tree / RF / XGBoost** CV helpers, **`run_xgb_tune_holdout.R`** (tune on train, score held-out test), and **`build_cv_rmse_results_md.R`** → **`CV_RMSE_RESULTS.md`**. **`data/README.md`**, **`config/README.md`**, and **`AI_USAGE.md`** document data layout, config, and tool-assisted work.
