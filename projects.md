...
**Repo (R / renv / GNU make):** Numbered scripts **`scripts/01_clean-data.R`** … **`08_evaluate.R`** — **pool** five Stata PUFs, harmonize year-suffixed names, apply exclusions, **EDA**, then **encode** categoricals (nominal one-hot where needed; ordinal/binary left as integers after **MEPS sentinel → NA → max+1** “missing” codes). **`make data`** runs the cleaning → EDA → processed Parquet path; **`make cv-local`** / **`make cv`** (SLURM) plus **`05_combine_cv.R`** build the CV leaderboard; **`make train-local`** / **`make train`** fits the chosen **XGBoost** model to **`models/xgboost/model.rds`**. **`make evaluate`** applies **`07_prep-test.R`** + **`08_evaluate.R`** to **`test.xlsx`** at the repo root. **`README.md`** and **`AI_USAGE.md`** document the pipeline and tool-assisted work; code is **MIT-licensed**.
